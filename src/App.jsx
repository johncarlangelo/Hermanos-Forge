...
import { Download, Settings, Clock, Trash2, FolderOpen, Video, Music, CheckCircle2, AlertCircle, Copy, Scissors, Menu, Terminal } from 'lucide-react';
    ...
    <div className="min-h-screen w-full bg-background">
      <div className="max-w-6xl mx-auto w-full flex flex-col p-6 min-h-screen">
        ...
        <div className="flex flex-1 gap-6 mt-6">
          {/* Sidebar */}
          <aside className={`${isSidebarCollapsed ? 'w-[72px]' : 'w-56'} flex-shrink-0 flex flex-col gap-2 transition-all duration-300 overflow-hidden`}>
            <button
              onClick={() => setIsSidebarCollapsed(!isSidebarCollapsed)}
              className={`flex items-center px-4 py-3 rounded-xl transition-all text-textSecondary hover:text-white hover:bg-surfaceHover mb-2 ${isSidebarCollapsed ? 'justify-center' : ''}`}
              title={isSidebarCollapsed ? "Expand Sidebar" : "Collapse Sidebar"}
            >
              <Menu className={`w-5 h-5 flex-shrink-0 ${isSidebarCollapsed ? '' : 'mr-3'}`} />
              {!isSidebarCollapsed && <span className="font-medium whitespace-nowrap">Collapse</span>}
            </button>

            <button
              onClick={() => setActiveTab('download')}
              className={`flex items-center px-4 py-3 rounded-xl transition-all ${activeTab === 'download' ? 'bg-primary text-white shadow-md' : 'bg-surface text-textSecondary hover:text-white hover:bg-surfaceHover'} ${isSidebarCollapsed ? 'justify-center' : ''}`}
              title={isSidebarCollapsed ? "Download" : ""}
            >
              <Download className={`w-5 h-5 flex-shrink-0 ${isSidebarCollapsed ? '' : 'mr-3'}`} />
              {!isSidebarCollapsed && <span className="font-medium whitespace-nowrap">Download</span>}
            </button>
            <button 
              onClick={() => setActiveTab('mass-stitch')}
              className={`flex items-center px-4 py-3 rounded-xl transition-all ${activeTab === 'mass-stitch' ? 'bg-primary text-white shadow-md' : 'bg-surface text-textSecondary hover:text-white hover:bg-surfaceHover'} ${isSidebarCollapsed ? 'justify-center' : ''}`}
              title={isSidebarCollapsed ? "Mass Clip" : ""}
            >
              <Scissors className={`w-5 h-5 flex-shrink-0 ${isSidebarCollapsed ? '' : 'mr-3'}`} />
              {!isSidebarCollapsed && <span className="font-medium whitespace-nowrap">Mass Clip</span>}
            </button>

            <div className="mt-auto pt-4">
              <button
                onClick={() => window.electronAPI && window.electronAPI.openLogsWindow()}
                className={`flex items-center px-4 py-3 rounded-xl transition-all w-full text-textSecondary hover:text-white hover:bg-surfaceHover ${isSidebarCollapsed ? 'justify-center' : ''}`}
                title={isSidebarCollapsed ? "Show Logs" : ""}
              >
                <Terminal className={`w-5 h-5 flex-shrink-0 ${isSidebarCollapsed ? '' : 'mr-3'}`} />
                {!isSidebarCollapsed && <span className="font-medium whitespace-nowrap">Show Logs</span>}
              </button>
            </div>
          </aside>
            ...
            <AnimatePresence mode="wait">
              {activeTab === 'mass-stitch' ? (
                <motion.div
                  ...
                  className="h-full"
                >
                  <MassClipDownloader onStitchSuccess={(filepath, clips) => {
                const validClips = clips.filter(c => c.url.trim());
                if (validClips.length > 0) {
                  const newItems = validClips.map((c, i) => ({
                    id: Date.now() + i,
                    url: c.url.trim(),
                    filename: filepath.split('\\').pop() + ` (Clip ${i + 1})`,
                    filepath,
                    type: 'stitch',
                    date: new Date().toLocaleString()
                  }));
                ...
                >
                  ...
                  >
                    ...
                    <div className="flex-1 overflow-y-auto pr-2 space-y-3">
                      {history.filter(h => historyTab === 'stitch' ? h.type === 'stitch' : h.type !== 'stitch').length === 0 ? (
                        ...
                        history.filter(h => historyTab === 'stitch' ? h.type === 'stitch' : h.type !== 'stitch').map((item) => (
                          <motion.div
                            key={item.id}
                          ...
                          >
                            <div className="flex items-center overflow-hidden mr-4">
                              <div className={`p-3 rounded-lg mr-4 ${item.type === 'mp4' ? 'bg-blue-500/10 text-blue-400' : item.type === 'stitch' ? 'bg-green-500/10 text-green-400' : 'bg-purple-500/10 text-purple-400'}`}>
